...
| 显示延迟 | 每分钟为每个在线节点请求一次近 1 小时延迟记录（最多 4 个并发，标签页隐藏时暂停） |
| 卡片延迟线路数 | 未指定线路时，每张卡片与列表显示的前几条线路，1–6，默认 3 |
| 显示的延迟线路 | 线路名称，用逗号（`,` 或 `，`）隔开，按填写顺序显示，如 `浙江电信,浙江联通,浙江移动`；不区分大小写，节点没有的线路会跳过；留空则显示前几条 |
| 默认时间范围 | 详情页历史图默认范围 |
